...
description: "API key and scoped token authentication for the Limitless Exchange API"
---
...
Every request is signed with HMAC-SHA256. The server verifies the signature using the token's secret.
...
Scoped API tokens require a partner setup. Contact [hey@limitless.network](mailto:hey@limitless.network) to get started. Once enabled, you can derive tokens via `POST /auth/api-tokens/derive` while authenticated with Privy.
