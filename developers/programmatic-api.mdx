...
The programmatic API is focused on **account creation**, **delegated order signing**, and **order management**.

- There is no partner redemption endpoint for server wallet sub-accounts.
- The SDK delegated order services do not include a `redeemPositions` helper.
- There is no partner withdrawal endpoint or partner-facing app UI for managing partner-created sub-accounts.
