...
For EOA mode, the end-to-end flow is:

1. **Register the user's wallet** — call `POST /profiles/partner-accounts` with the user's wallet ownership headers. The response includes a `profileId`.
2. **User signs orders** — the user signs the EIP-712 order with their private key. Set `maker` and `signer` to the user's wallet address.
3. **Submit the order** — call `POST /orders` with `onBehalfOf` set to the user's `profileId` from step 1. Do **not** send `ownerId` — the server sets it automatically.
