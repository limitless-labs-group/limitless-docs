...
As of SDK v1.0.6, all three SDKs provide helper methods for server wallet claim (redeem) and withdrawal. See the SDK getting-started pages for details.
...
<Info>
  Server-wallet redeem and withdraw helpers are available in the TypeScript, Go, and Rust SDKs. Python integrations should call the REST endpoints directly for those two routes for now.
</Info>
...
<CardGroup cols={4}>
  ...
  <Card title="Rust" icon="code" href="/developers/sdk/rust/api-tokens">
