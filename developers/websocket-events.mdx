...
<Note>
  `marketResolved` is also emitted to existing `market:{slug}` room subscribers automatically — you don't need a separate lifecycle subscription to receive resolution events for markets you're already watching.
</Note>
