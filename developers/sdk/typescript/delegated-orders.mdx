...
<Note>
  `DelegatedOrderService` covers delegated **order placement** and **cancellation**. The current public docs do not document separate SDK helpers for **position redemption** or **withdrawal** on server wallet sub-accounts. See [Programmatic API - Redemption and withdrawal support](/developers/programmatic-api#redemption-and-withdrawal-support).
</Note>
